...
class Webinar extends Model
{
    ...
    public function getBriefAttribute()
    {
        ...
        //print_r(nicePriceWithTax($this->price));die;
        
        $harga = nicePriceWithTax($this->price);
        $harga_setelah_diskon = nicePriceWithTax($this->bestTicket(true));
        //print_r($harga_setelah_diskon);die;
...
//asmi start
            'price' => (isset($harga['price'])) ? $harga['price'] : 0.0,
            'tax' => (isset($harga['tax'])) ? $harga['tax']  : 0,
            'tax_with_discount' => ($this->price > 0 and $this->bestTicket() < $this->price) ? $harga_setelah_diskon['tax'] : 0,
            'best_ticket_price' => ($this->price > 0 and $this->bestTicket() < $this->price) ? round($harga_setelah_diskon['price']['bestTicket'], 3) : 0.0,
            'discount_percent' => $harga_setelah_diskon['price']['percent'],


            'course_page_tax' => ($this->price > 0 ) ? (!$this->activeSpecialOffer()) ? nicePriceWithTax($this->price)['tax'] :
                nicePriceWithTax(number_format($this->price - ($this->price * $this->activeSpecialOffer()->percent / 100), 2))['tax'] : 0
            ,
            'price_with_discount' => nicePrice(($this->activeSpecialOffer()) ? (
            number_format($this->price - ($this->price * $this->activeSpecialOffer()->percent / 100), 2))
                : $this->price),
            ...
            'discount_amount' =>
                ($this->price > 0 ) ? ((int)nicePriceWithTax($this->price)['price']
                    - (int)round(nicePriceWithTax($this->bestTicket(true)['bestTicket'])['price']
                    )) : 0.0,
//asmi end
