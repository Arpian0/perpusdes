...
function convertMinutesToHourAndMinuteWithComma($minutes)
{
    // return intdiv($minutes, 60) . ',' . (str_pad(($minutes % 60) / 6, 1, 0, STR_PAD_LEFT));
    return intdiv($minutes, 60) . ':' . (str_pad($minutes % 60, 2, 0, STR_PAD_LEFT));
}
...
function getOfflineBanksTitle()
{
    ...
    foreach (getOfflineBankSettings() as $bank) {
        if (isset($bank['title'])) {
            $titles[] = $bank['title'];
        }
...
function handlePriceFormat($price, $decimals = 0, $decimal_separator = ',', $thousands_separator = '.'): string
{
    if (is_numeric($price) and $price > 0) {
        $format = number_format($price, 0, '$decimal_separator', $thousands_separator);
...
function handlePrice($price, $showCurrency = true, $format = true, $coursePagePrice = false, $user = null, $showTaxInPrice = false)
{
    ...
    if(intval($price)<1){
        //print_r(intval($price));die;
        return intval($price);
        //return 'Free';
    }

    if (is_numeric($price) && $priceDisplay != 'only_price') {
...
{
    $btn = \App\Models\NavbarButton::where('role_id', $roleId)
    ->where('for_guest', $forGuest)
    ->first();
    if(isset($btn->url)){
        if($btn->url=='/admin') $btn->url='/manajer';$btn->title='Panel Admin';
    }
